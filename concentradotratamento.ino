...
//float temperatura_enviada, distancia_enviada, ph_enviada, nenvio_temperatura, nenvio_distancia, nenvio_ph =0;
//float vazao_enviada, poco1_enviada, poco2_enviada, nenvio_vazao, nenvio_poco1, nenvio_poco2  = 0;
//float turbides_enviada,nenvio_turbides, cinza_enviada, nenvio_cinza, nenvio_ronda =0;
//float vazaosaida, vazaosaida_enviada, nenvio_vazaosaida = 0;

float temperatura_enviada, nenvio_temperatura =0;
float distancia_enviada, nenvio_distancia =0;
float ph_enviada, nenvio_ph =0;
float turbides_enviada, nenvio_turbides =0;
float cinza_enviada, nenvio_cinza =0;
float poco1_enviada, nenvio_poco1 =0;
float poco2_enviada, nenvio_poco2 =0;
float vazaosaida_enviada, nenvio_vazaosaida =0;
float  distancia,distancia2;
String ronda_enviada  = "";
float nenvio_ronda = 0;
...
*/
  ...
  if ((msgrecebidalora != "") || (msgrecebidawifi != "")) {
    ...
    if (identificador == VARIABLE_LABEL_CHAMINECINZA) {
      ...
      if (valor != cinza_enviada ) {
        //if ((valor > 0 ) && (valor < 15)){
          ...
          //googlesend(VARIABLE_LABEL_CHAMINECINZA, valor);
          cinza_enviada = valor;
          nenvio_cinza =0;
        //}        
      }else {
   ...
   //poco1
    if (identificador == VARIABLE_LABEL_VAZAOPOCO1) {
      valor = valorstring.toFloat(); // centimetro
      if (valor != poco1_enviada) {
       if ((valor >= 0) && (valor <=900)){
        sendValuesmqtthttp(valor, VARIABLE_LABEL_VAZAOPOCO1);
        googlesend(VARIABLE_LABEL_VAZAOPOCO1, valor);
        poco1_enviada = valor;
        nenvio_poco1 = 0;
       }       
      } else {
          nenvio_poco1 = nenvio_poco1 + 1;
          if (nenvio_poco1 > 5) {
            if ((valor >= 0) && (valor <=900)){
              sendValuesmqtthttp(valor, VARIABLE_LABEL_VAZAOPOCO1);
              googlesend(VARIABLE_LABEL_VAZAOPOCO1, valor);
    ...
    //poco2
    if (identificador == VARIABLE_LABEL_VAZAOPOCO2) {
      valor = valorstring.toFloat(); // centimetro
      if (valor != poco2_enviada) {
       if ((valor >= 0) && (valor <=900)){
        sendValuesmqtthttp(valor, VARIABLE_LABEL_VAZAOPOCO2);
        googlesend(VARIABLE_LABEL_VAZAOPOCO2, valor);
        poco2_enviada = valor;
        nenvio_poco2 = 0;
       }       
      } else {
          nenvio_poco2 = nenvio_poco2 + 1;
          if (nenvio_poco2 > 5) {
            if ((valor >= 0) && (valor <=900)){
              sendValuesmqtthttp(valor, VARIABLE_LABEL_VAZAOPOCO2);
              googlesend(VARIABLE_LABEL_VAZAOPOCO2, valor);
    ...
    //vazaosaida
    if (identificador == VARIABLE_LABEL_VAZAOSAIDA) {
      valor = valorstring.toFloat(); // centimetro
      if (valor != vazaosaida_enviada) {
       if ((valor >= 0) && (valor <=900)){
        sendValuesmqtthttp(valor, VARIABLE_LABEL_VAZAOSAIDA);
        googlesend(VARIABLE_LABEL_VAZAOSAIDA, valor);
        vazaosaida_enviada = valor;
        nenvio_vazaosaida = 0;
       }       
      } else {
          nenvio_vazaosaida = nenvio_vazaosaida + 1;
          if (nenvio_vazaosaida > 5) {
            if ((valor >= 0) && (valor <=900)){
              sendValuesmqtthttp(valor, VARIABLE_LABEL_VAZAOSAIDA);
              googlesend(VARIABLE_LABEL_VAZAOSAIDA, valor);
              vazaosaida_enviada = valor;
